...
public class Connector {
    ...
    public void connect(SocketTaskPoolExecutor socketTaskExecutor, String hostName, int port) throws IOException {
        // 도메인 이름을 IP 주소로 변환해서 유효성 검증한다.
        // IP가 시스템의 네트워크 인터페이스(eth0)에 할당되어 있어야함 (ip addr로 확인가능)
        listenSocket.bind(new InetSocketAddress(hostName, port), 100);
